...
public class ProductDao {	
	...
	public ArrayList<Product> selectProductList(int beginRow, int rowPerPage, String main, String sub) throws Exception{
		 ...
		 String sql="SELECT p.product_no productNo, p.category_no categoryNo, p.product_name productName, p.product_stock productStock, p.product_price productPrice, p.product_status productStatus, p.product_info productInfo, p.updatedate, p.createdate FROM product p INNER JOIN category c ON p.category_no = c.category_no AND c.category_main_name = ? AND c.category_sub_name = ? ORDER BY createdate DESC LIMIT ?, ?";
		 if(main.equals("전체") && sub.equals("전체")) {
				sql="SELECT product_no productNo, category_no categoryNo, product_name productName, product_stock productStock, product_price productPrice, product_status productStatus, product_info productInfo, updatedate, createdate FROM product ORDER BY createdate DESC LIMIT ?, ?";
				PreparedStatement stmt = conn.prepareStatement(sql);
				...
				sql="SELECT p.product_no productNo, p.category_no categoryNo, p.product_name productName, p.product_stock productStock, p.product_price productPrice, p.product_status productStatus, p.product_info productInfo, p.updatedate, p.createdate FROM product p INNER JOIN category c ON c.category_main_name = ? AND p.category_no = c.category_no ORDER BY createdate DESC LIMIT ?, ? ";
				PreparedStatement stmt = conn.prepareStatement(sql);
				stmt.setString(1, main);
