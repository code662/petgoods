...
public class ProductDao {	
	...
	public ArrayList<Product> selectProductList(String sort, int beginRow, int rowPerPage, String main, String sub) throws Exception{
		 ...
		 if(main.equals("전체") && sub.equals("전체")) {
				...
				sql="SELECT p.product_no productNo, p.category_no categoryNo, c.category_main_name categoryMainname, c.category_sub_name categorySubname, p.product_name productName, p.product_stock productStock, p.product_price productPrice, COALESCE((p.product_price*(1-d.discount_rate)),p.product_price) productDiscountPrice, COALESCE(d.discount_rate,0) discountRate, p.product_status productStatus, p.product_info productInfo, COALESCE(orderCnt,0) orderCnt, p.updatedate, p.createdate FROM product p INNER JOIN category c ON c.category_main_name = ? AND p.category_no = c.category_no LEFT OUTER JOIN discount d ON p.product_no = d.product_no LEFT OUTER JOIN (SELECT o.product_no, COUNT(o.product_no) orderCnt FROM orders o GROUP BY o.product_no) oc ON p.product_no = oc.product_no "+sort+" LIMIT ?, ? ";
				PreparedStatement stmt = conn.prepareStatement(sql);
				stmt.setString(1, main);
